...
AddEventHandler(
    ...
    function()
        ...
        if a <= 0 then
            User:notify("error", "tu n'as pas de pioche")
        else
...
AddEventHandler(
    ...
    function()
        ...
        User:notify("success", "Tu as trouvé " .. itemData:getName())
...
AddEventHandler(
    ...
    function(source, num)
        ...
        if num == 0 or num == nil then
            User:notify("error", "Erreur de traitement")
            return
        end
        if num >= 4 or num == 0 then
            if Character:hasGroup("ourives") then
            else
                User:notify("error", "Vous ne pouvez pas traiter ce minerai")
                return
            end
        end
...
AddEventHandler(
    ...
    function(num)
        ...
        if num == 1 then
            ...
            User:notify("Tu traites [X" .. count .. "] pierres")
            Wait(1000)
            Inventory:addItem("generic_pedralavada", count)
            User:notify("Tu as reçu [X" .. count .. "] de pierres traitées")
            return
        end
        if num == 2 then
            Inventory:removeItem(-1, "generic_carvaobruto", 1)
            User:notify("tu traites [X1] de charbon")
            Wait(1000)
            Inventory:addItem("generic_carvaorefi", count)
            User:notify("Tu as reçu [X" .. count .. "] de charbon rafiné")
            return
        end
        if num == 3 then
            Inventory:removeItem(-1, "generic_cobrebruto", 1)
            User:notify("tu traites [X" .. count .. "] de cuivre")
            Wait(1000)
            Inventory:addItem("generic_provision_disco_ammolite", count)
            User:notify("Tu as reçu [X" .. count .. "] de lingot de cuivre")
            return
        end
        if num == 4 then
            Inventory:removeItem(-1, "generic_ourobruto", 3)
            User:notify("tu traites [X" .. count .. "] d'or")
            Wait(1000)
            print(count * 10)
