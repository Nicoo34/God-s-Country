...
local promptGroupName_pickup = CreateVarString(10, "LITERAL_STRING", "Transport de produits")
local promptGroupName_deliver = CreateVarString(10, "LITERAL_STRING", "Emplacement de la cargaison")
local promptGroupName_deliver_error = CreateVarString(10, "LITERAL_STRING", "où est ma cargaison?")
                ...
                if #(pCoords - closestPickupSpotCoords) <= 1.0 then
                    ...
                    if PromptIsJustPressed(prompt_pickup) then
                        ...
                        if canSpawn then
                            ...
                            cAPI.notify("success", "une charrette vous attend dehors")

                            print(vehicle)
                        else
                            cooldown = 5
                            cAPI.notify("error", "Il y a déjà un chariot dehors")
                        end
                    end
                end
            end
                    ...
                    if canDeliver then
                        ...
                        if PromptIsJustPressed(prompt_deliver) then
                            ...
                            if lV + 1 <= #deliverySpots then
                                ...
                                cAPI.notify("speech", "Cargaison livrée, allez au prochain magasin")
                            else
                                DecorRemove(vehPedIsIn, "job_transport_vehicle_num_cargo")
                                -- SetVehicleBodyHealth(vehPedIsIn, 0.0)
                                cAPI.notify("speech", "Toutes les cargaisons ont été livrées")
                            end
                        end
                    else
...
function InitPrompt()
    ...
    PromptSetText(prompt_pickup, CreateVarString(10, "LITERAL_STRING", "Prise de service"))
    ...
    PromptSetText(prompt_deliver, CreateVarString(10, "LITERAL_STRING", "Fin de service"))
