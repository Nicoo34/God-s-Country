...
local varString_carteiro = CreateVarString(10, "LITERAL_STRING", "Facteur")
    ...
    function()
        while true do
            ...
            if timer == 0 then
                ...
                if steps >= 1 then
                    DrawText("Tu as " .. Cards .. " cartes", 0.925, 0.96, 0.4, 0.4, false, 255, 255, 255, 145, 1, 7)
                end
                ...
                elseif steps == 1 then
                    ...
                    if Cards < 15 then
                        if GetDistanceBetweenCoords(getPosts().x, getPosts().y, getPosts().z, posPlayer[0], posPlayer[1], posPlayer[2]) < 2.5 then
                            PromptSetText(postManPrompt, CreateVarString(10, "LITERAL_STRING", "ramasser des cartes"))
                            PromptSetActiveGroupThisFrame(postManGroup, varString_carteiro)
                            if PromptHasHoldModeCompleted(postManPrompt) then
                                Citizen.Wait(100)
                ...
                elseif steps == 2 then
                    if DoesBlipExist(blip[steps]) then
                        CreateBlip(steps, SettedRoute, "livrer vos lettres", -1656531561)
                    else
                        if Cards ~= 0 then
                            if GetDistanceBetweenCoords(SettedRoute.x, SettedRoute.y, SettedRoute.z, posPlayer[0], posPlayer[1], posPlayer[2]) < 4.5 then
                                PromptSetText(postManPrompt, CreateVarString(10, "LITERAL_STRING", "jouer aux cartes"))
                                PromptSetActiveGroupThisFrame(postManGroup, varString_carteiro)
                                if PromptHasHoldModeCompleted(postManPrompt) then
                                    Citizen.Wait(100)
                                    Cards = Cards - 1
                            ...
                            notifyHelp("Vos lettres sont parties, attendez 20 minutes et retournez à la poste.")
                            DeleteBlips()
                            timer = 1200
                            steps = 0
                            postUsing = 0
        ...
        while true do
            ...
            for i = 1, #WorkStations, 0 do
                ...
                if blip["start" .. i] == nil then
                    CreateBlip("start" .. i, WorkStations[i], "poster", 1861010125)
                end
...
function createPrompt()
    if PromptIsValid(postManPrompt) then
        ...
        PromptSetText(postManPrompt, CreateVarString(10, "LITERAL_STRING", "trouver un emploi"))
