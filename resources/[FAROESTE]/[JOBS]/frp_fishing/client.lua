...
Citizen.CreateThread(
    function()
        ...
        while true do
            if FISHING_GET_MINIGAME_STATE() == 1 then
                PromptSetActiveGroupThisFrame(fishing_data.prompt_prepare_fishing.group, CreateVarString(10, "LITERAL_STRING", "Pêcher"))
            end

            if FISHING_GET_MINIGAME_STATE() == 6 then
                PromptSetActiveGroupThisFrame(fishing_data.prompt_waiting_hook.group, CreateVarString(10, "LITERAL_STRING", "en attendant d'attraper"))
            end

            if FISHING_GET_MINIGAME_STATE() == 7 then
                fishing_data.fish.weight = FISHING_GET_F_(8)
                PromptSetActiveGroupThisFrame(fishing_data.prompt_hook.group, CreateVarString(10, "LITERAL_STRING", "tirer du poisson"))
            end

            if FISHING_GET_MINIGAME_STATE() == 12 then
                PromptSetActiveGroupThisFrame(fishing_data.prompt_finish.group, CreateVarString(10, "LITERAL_STRING", "Poisson " .. fishs[GetEntityModel(FISHING_GET_FISH_HANDLE())] .. " Kg" .. (fishing_data.fish.weight * 54.25)))
            end
            Citizen.Wait(0)
        end
    end
...
function prepareMyPrompt()
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "changer d'appât"))
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "Finir pêche"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_prepare_fishing.group)
    ...
    prompt = PromptRegisterBegin()
    PromptSetControlAction(prompt, GetHashKey("INPUT_ATTACK")) -- MOUSE LEFT CLICK
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "Accrocher"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "Collecter"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_waiting_hook.group)
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "tirez la ligne"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    ...
    prompt = PromptRegisterBegin()
    PromptSetControlAction(prompt, 0x8FFC75D6) -- LEFT SHIFT
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "Couper la ligne"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_hook.group)
    ...
    prompt = PromptRegisterBegin()
    PromptSetControlAction(prompt, GetHashKey("INPUT_ATTACK")) -- MOUSE LEFT CLICK
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "attraper le poisson"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "remettre le poisson à la mer"))
    PromptSetEnabled(prompt, true)
    PromptSetVisible(prompt, true)
    PromptSetHoldMode(prompt, false)
    PromptSetGroup(prompt, fishing_data.prompt_finish.group)
