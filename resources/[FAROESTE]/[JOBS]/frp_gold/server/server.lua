...
function()	
    ...
    local count = math.random(1, 3)
            ...
            Inventory:addItem("rock", count * 2)
            
            User:notify("item", "rock", count * 2)
...
AddEventHandler(
    ...
    function()
        ...
        local rock = false
        local HaveRock = true
        local nuggetcount = math.random(1, 2)

     while HaveRock do

        if Inventory:getItemAmount("rock") >= 3 then
            Inventory:removeItem(-1, "rock", -3)
...
AddEventHandler(
    ...
    function()
     ...
     while HaveGoldNugget do
        ...
        if Inventory:getItemAmount("goldnugget") >= 1 then
            Inventory:removeItem(-1, "goldnugget", 1)
