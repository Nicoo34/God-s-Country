...
AddEventHandler(
    ...
    function(model, name)
        ...
        print("User " .. User:getId() .. " a essayé d'acheter un cheval (" .. model .. "," .. name .. ")")

        if Character:getHorses() ~= nil then
            if #Character:getHorses() > 2 then
                User:notify("error", "Limite de chevaux atteinte.")
                return
            end
        end
        ...
        if Character:getInventory():removeItem("money", priceMoney) or Character:getInventory():removeItem("generic_gold", priceGold) then
            ...
            User:notify("success", "tu as acheté le " .. name .. " pour " .. priceMoney or priceGold)
        else
            User:notify("error", "Pas assez d'argent ou d'or suffisant.")
        end
    end
)
