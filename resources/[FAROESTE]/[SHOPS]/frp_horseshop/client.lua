...
function tryToBuyHorse(horseModel, buyWithGold)
	local horseName = cAPI.prompt('nomme ton cheval:', 'Cavalo')
	WarMenu.CloseMenu()
	if horseName ~= '' then
...
Citizen.CreateThread(
	function()
		for _, values in pairs(shops) do
			...
			SetBlipScale(b, 0.2)
			Citizen.InvokeNative(0x9CB1A1623062F402, b, 'Ecuries')
			...
			for _, values in pairs(shops) do
				...
				if #(pCoords - vec3(x, y, z)) <= 5 then
					if IsPedOnMount(ped) then
						PromptSetText(prompt, CreateVarString(10, 'LITERAL_STRING', 'sauvegarder le cheval'))
						local horseGroupName = CreateVarString(10, 'LITERAL_STRING', 'Magasin')
					...
					elseif not WarMenu.IsMenuOpened('horses') then
						PromptSetText(prompt, CreateVarString(10, 'LITERAL_STRING', 'acheter cheval'))
						local horseGroupName = CreateVarString(10, 'LITERAL_STRING', 'Magasin')
						PromptSetActiveGroupThisFrame(promptGroup, horseGroupName)
						if PromptHasHoldModeCompleted(prompt) then
							Citizen.Wait(1000)
...
Citizen.CreateThread(
	function()
		WarMenu.CreateMenu('horses', 'Ecuries')
		WarMenu.SetSubTitle('horses', 'Sélectionner une option')
		WarMenu.CreateSubMenu('horse_shop', 'horses', 'Acheter un cheval')
		WarMenu.CreateSubMenu('my_horses', 'horses', 'mes chevaux')
		while true do
			if WarMenu.IsMenuOpened('horses') then
				WarMenu.MenuButton('Acheter un cheval', 'horse_shop')

				if WarMenu.MenuButton('mes chevaux', 'my_horses') then
			...
			if WarMenu.IsMenuOpened('horse_shop') then
				for _, horseData in pairs(buyableHorses) do
					if not IsControlPressed(2, 0) then
						...
						if WarMenu.Button(horseData['Text'], '~g~' .. horseData['priceGold'] .. ' d\'or') then
							tryToBuyHorse(horseData['Model'], false)
						end
					end
				end
