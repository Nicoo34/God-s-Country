...
local blips = {
    { name = 'Animalerie', sprite = -1646261997,x = -273.51, y = 689.26, z = 113.41 }
}
...
AddEventHandler( 'FRP:PET:spawndog', function ( dog, isInShop )
	...
	if dogspawn[idOfThedog] == nil then
		...
		Citizen.InvokeNative(0x9CB1A1623062F402, blip, 'votre animal')
	...
	if dogspawn[idOfThedog] then
		...
		if isInShop then
			...
			if EntityIsDead then
				ShowNotification( "un mèdecin a soigné votre chien" )
