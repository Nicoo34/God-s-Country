            ...
            if cAPI.IsPlayerHorseActive() then
                if not isHorseActivationBlocked then
                    if IsPedInjured(playerHorse) then
                        cAPI.notify("error", "Votre cheval a été blessé, vous ne pourrez pas l’appeler dans les deux prochaines minutes.")
        ...
        while true do
            ...
            if isHorseInWrithe then
                ...
                if IsPedInWrithe(playerHorse) then
                    cAPI.notify("alert", "Le cheval a été blessé, réanimé le !")
...
Citizen.CreateThread(
    function()
        while true do
            ...
            if diff > 1 then
                if isWanted then
                    DrawText("Vous êtes recherché pour " .. string.format("%.0f", math.max(diff / 1000, 0)) .. " seconde", 0.925, 0.96, 0.25, 0.25, false, 255, 255, 255, 145, 1, 7)
                end
            else
                if isWanted then
                    isWanted = false
