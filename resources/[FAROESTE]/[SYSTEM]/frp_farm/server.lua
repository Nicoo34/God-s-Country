...
AddEventHandler(
    ...
    function(seedTryingToPlant, farmAreaId, slot_id)
        ...
        if farmsInfo[farmAreaId][slot_id] ~= nil then
            User:notify("error", "Trouver un autre endroit pour planter!")
            return
        end
        ...
        if seedTryingToPlant ~= farmAreaId then
            User:notify("error", "Vous ne pouvez pas planter ce genre de graines ici !")
            return
        end
        ...
        if Inventory:getItemAmount(seedItem) <= 0 then
            User:notify("error", "vous avez besoin d'une graine à planter!")
            return
        end
    ...
    function(farmAreaId, slot_id)
        ...
        if (notReadyFarmsInfo[farmAreaId] and notReadyFarmsInfo[farmAreaId][slot_id]) or (farmsInfo[farmAreaId][slot_id][2] > os.time()) then
            ...
            if (notReadyFarmsInfo[farmAreaId] and notReadyFarmsInfo[farmAreaId][slot_id]) then
                print('ne peux pas être arroser maintenant')
            else
                print("attendre " .. farmsInfo[farmAreaId][slot_id][2] - os.time() .. "secondes avant d'arroser la plante")
                User.notify("Attention! si vous arrosez trop la plante peut mourir")
            end
            return
        end
...
AddEventHandler(
    ...
    function(farmAreaId, slot_id)
        ...
        if Inventory:addItem(plantItem, 1) then
            ...
            User:notify("error", "plus de place")
        end
    end
)
