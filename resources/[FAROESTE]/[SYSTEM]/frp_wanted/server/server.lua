    ...
    function(targetCoords, CityName, playerGender, suspectInstead)
        ...
        for i = 1, #PoliceON do
            ...
            if suspectInstead then
                TriggerClientEvent("FRP:TOAST:New", PoliceON[i].getSource(), "alert", "Shérif : Suspect trouvé dans " .. CityName)
            else
                TriggerClientEvent("FRP:TOAST:New", PoliceON[i].getSource(), "alert", "Shérif : Coups de feu par " .. playerGender .. " ont été signalés dans " .. CityName)
            end
        end
    end
)
