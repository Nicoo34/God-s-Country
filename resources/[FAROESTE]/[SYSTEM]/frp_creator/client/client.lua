            ...
            if vpcreator and not inCustomization then
                if groundCam == nil then
                  ...
                  --  createCam("selection")                  
                    ...
                    DestroyAllCams(true)
                    createCamera()
                    TriggerEvent('FRP:NOTIFY:Simple', 'Utilisez les fléches de votre clavier et appuyez sur Entrée pour choisir un type de personnage.', 15000)
                else
...
RegisterNUICallback(
    ...
    function(data)
        ...
        if data.id == 0 then
            ...
            if sex == "mp_male" then
                for k, v in pairs(MaleEyebrows) do
                    if MaleEyebrows[k].id == tonumber(data.id) then
                        SetPedHeadOverlay(PlayerPedId(), 1, overlayValue + 1, overlayOpacity)
                        Citizen.InvokeNative(0xD3A7B003ED343FD9, pedSelected, MaleEyebrows[k].hash, true, true, true)
                        EyebrowsUsing = MaleEyebrows[k].hash
                    end
                end
            end
        end
