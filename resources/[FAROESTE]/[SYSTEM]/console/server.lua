...
commands.addwhitelist = function(source, args)
    ...
    if userId then
        if API.setAsWhitelisted(userId, true) then
            commands.print(source, name .. " [" .. userId .. "] Ajouté à la whitelist!")
        else
            commands.print(source, name .. " [" .. userId .. "] est déjà whitelist")
        end
    else
...
commands.removewhitelist = function(source, args)
    ...
    if userId then
        if API.setAsWhitelisted(userId, false) then
            commands.print(source, name .. " [" .. userId .. "] supprimé de la whitelist!")
        else
            commands.print(source, name .. " [" .. userId .. "] n'est pas sur la whitelist")
        end
    else
...
commands.players = function(source, args)
    ...
    commands.print(source, "Connexion " .. #users .. "/32")
...
commands.playersbygroup = function(source, args)
    ...
    if group then
        ...
        commands.print(source, "Connexion " .. #users .. "/32 GROUPE: " .. group)
...
commands.setpriority = function(source, args)
    ...
    if user_id and priority then
        commands.print(source, "Priorité à l'utilisateur " .. user_id .. " Maintenant c'est " .. priority)
