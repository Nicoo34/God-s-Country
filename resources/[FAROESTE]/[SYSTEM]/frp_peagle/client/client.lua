            ...
            Citizen.Wait(1)
                ...
                if dst < 25 then  
                    if not Notified then
                        TriggerEvent('FRP:NOTIFY:Simple', 'Vous avez reçu un pigeon voyageur, attendez que le pigeon s\'approche de vous.', 5000)
                        Notified = true 
                        back = false                   
                    end
...
function InitiatePrompts()
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "recevoir un message"))
    PromptSetEnabled(prompt, true)
