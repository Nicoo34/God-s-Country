...
Citizen.CreateThread(
    function()
        ...
        PromptSetText(prompt_craft, CreateVarString(10, "LITERAL_STRING", "Produire"))
        ...
        PromptSetText(prompt_cancel, CreateVarString(10, "LITERAL_STRING", "Annuler"))
        ...
        while true do
            ...
            if not craftingNuiIsOpen then
                if not isCrafting then
                    if IsControlJustPressed(0, 0x4BC9DABB) and not alreadyRequestedCraftingNui then
...
function GetNearestAlhambicPosition(radius)
    local modelal = {
        "p_alcoholcontainer",
    }
    ...
    for _, model in pairs(modelal) do
        local entity = GetClosestObjectOfType(playerPosition, radius, GetHashKey(model), 0, 0, 0)
        if DoesEntityExist(entity) then
            position = GetEntityCoords(entity)
            break
...
AddEventHandler(
    ...
    function()
        ...
        local hasAlhambicNearby = GetNearestAlhambicPosition(DEFAULT_CRAFTING_POSITION_RADIUS) ~= nil
...
AddEventHandler(
    ...
    function(r)
        ...
        local lookAtPosition = nearestCraftingConfig.position or GetNearestAlhambicPosition(DEFAULT_CRAFTING_POSITION_RADIUS) or playerPosition
