...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if Character:hasGroupOrInheritance("admin") then
            ...
            User:notify("error", "Tu n'as pas la permission!")
        end
    end
)
    ...
    function(source, args, rawCommand)
        ...
        if Character:hasGroupOrInheritance("admin") and args[1] then
            ...
            if UserTarget ~= nil then
                ...
                User:notify("error", "joueur inconnue!")
            end
        else
            User:notify("error", "Tu n'as pas la permission!")
        end
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        local Character = User:getCharacter()
        if Character:hasGroupOrInheritance("admin") then
            local fcoords = cAPI.prompt(source, "Coordonées:", "")
            if fcoords == "" then
                return
            end
            local coords = {}
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if Character:hasGroupOrInheritance("admin") and args[1] then
            ...
            if args[2] ~= nil then
                API.setBanned(sourcePlayer, args[1], "Bannis pour " .. table.concat(args, " ", 2))
            else
                API.setBanned(sourcePlayer, args[1], "Bannis")
            end
        else
            User:notify("error", "Tu n'as pas la permission!")
        end
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if Character:hasGroupOrInheritance("admin") and args[1] then
            local sourcePlayer = API.getUserFromUserId(parseInt(args[1])):getSource()
            API.kick(sourcePlayer, "Vous avez été kick.")
        else
            User:notify("error", "Tu n'as pas la permission!")
        end
    end
    ...
    function(source, args, rawCommand)
        ...
        if Character:hasGroupOrInheritance("admin") then
            ...
            if UserTarget ~= nil then
                ...
                TriggerClientEvent("chatMessage", source, args[2] .. " XP ajouté au joueur " .. UserTarget:getCharacter():getName())
            end
        else
            User:notify("error", "Tu n'as pas la permission!")
        end
...
RegisterCommand(
    ...
    function(source, args, raw)
        ...
        if Character:hasGroupOrInheritance("admin") then
            ...
            if UserTarget ~= nil then
                UserTarget:getCharacter():varyExp(-(tonumber(args[2])))
                TriggerClientEvent("chatMessage", source, args[2] .. " XP retiré au joueur " .. UserTarget:getCharacter():getName())
            end
        else
            User:notify("error", "Tu n'as pas la permission!")
        end
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if #args < 2 and not tonumber(args[2]) then
            User:notify("error", "syntaxe: /item id quantité")
            return
        end
            ...
            if #args >= 3 then
                ...
                if UserTarget ~= nil then
                    ...
                    if CharacterTarget ~= nil then
                        ...
                        API.logs("./savedata/giveitem.txt",  os.date() .. " | [AdminID]: " .. Character:getId() .. " / [TargetID]: " .. CharacterTarget:getId() .. " [FONCTION]: AddItem / [NOM]: " .. args[2] .. " / Quantité " .. tonumber(args[3]))
                    else
                        User:notify("error", "L'utilisateur n'a pas encore choisi de personnage.!")
                    end
                else
                    User:notify("error", "Invalide!")
                end
            else
                if #args == 2 then
                    Character:getInventory():addItem(args[1], tonumber(args[2]))
                    API.logs("./savedata/giveitem.txt", os.date() .. " | [AdminID]: " .. Character:getId() .. " / [TargetID]: " .. args[1] .. " [FUNCTION]: AddItem / [NOM]: " .. args[1] .. " / Quantité " .. tonumber(args[2]))
                end
            end
        else
            User:notify("error", "Tu n'as pas la permission!")
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if #args < 2 and not tonumber(args[2]) then
            User:notify("error", "syntax: /item id quantité")
            return
        end
            ...
            if #args >= 3 then
                ...
                if UserTarget ~= nil then
                    ...
                    if CharacterTarget ~= nil then
                        ...
                        User:notify("error", "Pas de joueur valide!")
                    end
                else
                    User:notify("error", "Erreur!")
                end
            else
                if #args == 2 then
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if Character:hasGroupOrInheritance("admin") then
            local x, y, z = cAPI.getPosition(source)
            API.prompt(source, "Coordonnées:", string.format("%.3f", x) .. "," .. string.format("%.3f", y) .. "," .. string.format("%.3f", z))
        else
            User:notify("error", "Tu n'as pas la permission!")
        end
    end
        ...
        if Character:hasGroupOrInheritance("admin") then
            if args[1] == "add" or args[1] == "remove" then
                ...
                if UserTarget ~= nil then
                    ...
                    API.logs("./savedata/group.txt",  os.date() .. " | [AdminID]: " .. Character:getId() .. " / [TargetID]: " .. args[2] .. " [FONCTION]: ".. args[1] , args[3])
                else
                    User:notify("error", "Utilisateur invalide!")
                end
            end
        else
            User:notify("error", "Tu n'as pas la permission!")
...
RegisterCommand(
    ...
    function(source, args, rawCommand)
        ...
        if #PoliceON <= 0 then
            User:notify("alert", "Pas de FDO en service!")
        end
        User:notify("alert", "En ce moment il y a " .. #PoliceON .. " FDO en service!")
    end
)
        ...
        if Character:hasGroupOrInheritance("admin") or Character:hasGroupOrInheritance("medic") then
            if args[1] ~= nil then
                ...
                if tplayer ~= nil then
                    ...
                    API.logs("./savedata/revive.txt",  os.date() .. " | [AdminID]: " .. Character:getId() .. " / [TargetID]: " .. args[1] .. " [FONCTION]: REVIVE")
                end
            else
