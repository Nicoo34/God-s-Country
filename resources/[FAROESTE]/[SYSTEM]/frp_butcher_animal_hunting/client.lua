...
local prompt
local prompt_shop
local prompt_name = CreateVarString(10, "LITERAL_STRING", "Boucher")
...
AddEventHandler(
    ...
    function(type, entityModel)
        ...
        if type == 1 then
            TriggerEvent("FRP:NOTIFY:Simple", "je recherche un(e) " .. GetModelAnimalName(entityModel) .. ", Je te récompenserai si tu me l'apportes", 5000)
        elseif type == 2 then
            TriggerEvent("FRP:NOTIFY:Simple", "Le boucher est toujours à la recherche d'un(e) " .. GetModelAnimalName(entityModel), 5000)
        elseif type == 3 then
            cAPI.notify("error", "Terminez la chasse en cours pour pouvoir en commencer une autre!")
        end
    end
)
...
function HandlePrompts()
    ...
    if PromptIsJustPressed(prompt_shop) then
        ...
        TriggerEvent("FRP:SHOP:OpenShopByName", "Boucher")
    end
end
...
function InitiatePrompts()
    ...
    PromptSetText(prompt_shop, CreateVarString(10, "LITERAL_STRING", "vendre des produits d'épicerie"))
    ...
    PromptSetText(prompt, CreateVarString(10, "LITERAL_STRING", "Chasse spécifique"))
