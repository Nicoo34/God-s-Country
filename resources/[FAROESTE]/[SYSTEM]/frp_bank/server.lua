    ...
    function(amount)
        ...
        if amount ~= nil then
            if _amount == nil or _amount <= 0 or _amount > tonumber(call) then
                User:notify("error", "Quantité invalide!")
                return
            else
...
RegisterCommand(
    ...
    function(source, args)
        ...
        if amount ~= nil and playersend ~= nil then
            ...
            TriggerClientEvent("chat:addMessage", source, {args = {"^1SYSTEMA", "ID ou valeur invalide."}})
        end
    end
)
    ...
    function(source, id, amount)
        ...
        if amount ~= nil then
            if _amount == nil or _amount <= 0 or _amount > tonumber(call) then
                ...
                TriggerClientEvent("chatMessage", source, "^1SISTEMA", {255, 255, 255}, "Vous avez donné " .. amount * 100 .. " à " .. tname .. "")
            end
        end
    end
)
...
AddEventHandler(
    ...
    function(source, amount, name)
        ...
        if Character ~= nil then
            Inventory:addItem("money", amount * 100)
            TriggerClientEvent("chatMessage", source, "^1SISTEMA", {255, 255, 255}, "Vous avez reçu $" .. amount * 100 .. " de " .. _name .. "")
        end
    end
)
