      ...
      for k, v in pairs(Banks) do
        if Vdist(coords, v) < 2 then
          ...
          if weaponHash == hashUnarmed then
            DrawTxt("Appuyez sur (ALT) pour accéder à votre coffre-fort", 0.85, 0.95, 0.4, 0.4, true, 255, 255, 255, 255, true, 10000)
            if IsControlJustPressed(0, 0xE8342FF2) then
              if not cAPI.IsWanted() then
                ...
                TriggerEvent('FRP:NOTIFY:Simple', 'vous ne pouvez pas accéder à votre banque. ', 10000)
              end
            end
          end
        end
...
AddEventHandler(
  ...
  function(player, amount, nome)
    ...
    if distance ~= -1 and distance <= 3.0 then
      ...
      TriggerEvent("chatMessage", "^1SISTEMA", {255, 255, 255}, "personne autour")
    end
  end
)
  ...
  function(toPlayer, amount)
    if (IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
      ...
      if (playing ~= false) then
        ...
        TriggerEvent("chatMessage", "", {255, 0, 0}, "^1~joueur non trouvé.")
      end
    else
      TriggerEvent("chatMessage", "", {255, 0, 0}, "^1tu n'es pas proche de cette personne!")
    end
  end
)
