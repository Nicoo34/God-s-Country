...
function Process(action, start, tick, finish)
    ...
    if not IsEntityDead(PlayerPedId()) or vp_action.useWhileDead then
        if not isDoingAction then
            ...
            exports['frp_notify']:SendAlert('error', 'Déjà en train de faire une action', 1000)
        end
    else
        exports['frp_notify']:SendAlert('error', 'Impossible d\'effectuer une action en étant mort', 1000)
    end
end
