...
function HandleAsInjured(fatal)
	...
	if fatal == true then
		...
		fullDeathAtGameTime = GetGameTimer() + 3000 * 100
			...
			if pedHealth <= 0 then
				...
				if fullDeathTimeDiff <= 0 then
					...
					if PromptHasHoldModeCompleted(prompt_respawn) then
						...
						local respawnplace = SetEntityCoords(PlayerPedId())
						...
						NetworkResurrectLocalPlayer(coords, 59.95, true, true, false)
					...
					DrawTxt("~e~INCONSCIENT", 0.50, 0.04, 0.8, 0.8, true, 255, 255, 255, 255, true)
		...
		while isBadlyInjuried do
			...
			if pedHealth <= 0 then
					...
					if timesLeftPlayerCanGetUp > 0 then
						...
						if PromptIsEnabled(prompt_getup) == 0 then
							if timeTillfirstReviveDiff <= 0 then
								...
								PromptSetMashWithResistanceMode(prompt_getup, 10, 3.0 - (timesLeftPlayerCanGetUp * 0.75), 0)
				...
				DrawSprite("menu_textures", "translate_bg_1a", 0.50, 0.10, 0.20, 0.08, 0.8, 0, 0, 0, 250, 1)
				DrawTxt("~e~EVANOUI", 0.50, 0.07, 0.8, 0.8, true, 255, 255, 255, 255, true)
			else
				if fakeGameplayCam ~= nil then
					--[[
								A camera por algum motivo não existe enquanto
...
function SetAsDrunk(bool)
	...
	Citizen.InvokeNative(0xCB9401F918CB0F75, ped, "chain_gang_legs", bool, -1)
	Citizen.InvokeNative(0x437C08DB4FEBE2BD, ped, "chain_gang_legs", bool, -1)
end
...
function InitiateRespawnPrompt()
	...
	PromptSetText(prompt_respawn, CreateVarString(10, "LITERAL_STRING", "Revivre"))
...
function InitiateGetUpPrompt()
	...
	PromptSetText(prompt_getup, CreateVarString(10, "LITERAL_STRING", "Relever"))
	...
	PromptSetText(prompt_giveup, CreateVarString(10, "LITERAL_STRING", "Abandonner"))
