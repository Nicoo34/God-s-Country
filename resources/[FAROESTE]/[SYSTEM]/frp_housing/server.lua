...
AddEventHandler(
    ...
    function(house_id, withGold)
        ...
        if Character:hasGroupOrInheritance(house_id) then
            User:notify("error", "vous avez déjà loué cette résidence")
            return
        end
        ...
        if #rows > 0 then
            User:notify("error", "Cette résidence est déjà louée")
            return
        end
        ...
        if Inventory:getItemAmount(currencyItem) < currencyAmount then
            User:notify("error", "tu n'as pas assez de " .. (withGold and 'ouros' or 'dollares') .. " suffisant.")
            return
        end

        if Inventory:removeItem(-1, currencyItem, currencyAmount) then
            User:notify("success", "Maintenant cette résidence est sous ta gestion")
