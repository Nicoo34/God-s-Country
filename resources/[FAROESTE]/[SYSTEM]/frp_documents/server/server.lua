...
AddEventHandler("FRP:DOCUMENTS:requestCharInfo", function(TargetSource)
  ...
  --  local TargetSource = API.getUserFromUserId(parseInt(targetUserId)):getSource()
    ...
    local yes = cAPI.request(TargetSource, "O " ..Character:getName().." a demandé votre identification, voulez vous les montrer ?", 30)
    ...
    if yes then 
        ...
        User:notify("Le citoyen a refusé de présenter sa pièce d'identité")
    end
