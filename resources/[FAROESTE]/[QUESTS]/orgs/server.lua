            ...
            if Character then
                ...
                if not hasIlegalOrg then
                    ...
                    if Inventory:getItemAmount("money") < _ILEGAL_ORG_CREATION_PRICE then
                        User:notify("error", "Vous n'avez pas assez d'argent")
                        return
                    end

                    local org_name = cAPI.prompt(_source, "nom de l'organisation:", "")

                    if org_name == "" then
                        User:notify("error", "Vous n'avez pas spécifié de nom valide.")
                        return
                    end

                    if string.len(org_name) > 16 then
                        User:notify("error", "Le nom de l'organisation ne peut pas dépasser 16 caractères..")
                        return
                    end
                    ...
                    if Inventory:removeItem(-1, "money", _ILEGAL_ORG_CREATION_PRICE) then
                        ...
                        if org_id then
                            ...
                            User:notify("success", "Organisation créée!")
                        else
                            User:notify("error", "Une erreur s'est produite lors de la création de votre organisation :(")
                    ...
                    User:notify("error", "Vous êtes déjà dans une organisation")
                end
            end
        end
    end
            ...
            if Character then
                ...
                if my_member_rank == 1 then
                    ...
                    if org_type == "ilegal" then
                        ...
                        User:notify("success", "Votre organisation a été supprimée")
                    else
                        User:notify("error", "Cette organisation ne peut pas être supprimée.")
                    end
                else
                    User:notify("error", "Seul le propriétaire peut supprimer l'organisation")
                end
            end
        end
    end
                ...
                for _, mbm_d in pairs(org_members) do
                    ...
                    if member_id == my_member_id then
                        ...
                        if member_rank ~= 1 or member_rank == 1 and org_type ~= "ilegal" then
                            ...
                            User:notify("success", "vous avez quitté l'organisation")
                        else
                            User:notify("error", "Vous ne pouvez pas quitter votre propre organisation")
                        end
        ...
        if User then
            ...
            if Character then
                ...
                if not im_allowed_to_kick then
                    User:notify("error", "Vous ne faites plus partie de l'organisation ou vous n'êtes pas autorisé à expulser")
                    return
                end

                if not target_is_in_the_org then
                    User:notify("error", "Ce membre ne fait plus partie de l'organisation.")
                    return
                end
                ...
                if target_member_rank == 1 then
                    ...
                    User:notify("error", "membre de rang " .. rank_name .. ", ne peut pas être expulsé.")
                    return
                end

                Orgs.RemoveMember(org_id, target_member_id)

                User:notify("success", "Membre retiré de l'organisation.")
            end
        end
    end
)
    ...
    function(org_id, target_member_id)
        ...
        if User then
            ...
            if Character then
                ...
                if target_member_rank == 1 then
                    User:notify("error", "Vous ne pouvez pas modifier le rang de ce membre..")
                    return
                end
                ...
                if new_rank == 1 then
                    User:notify("error", "Le membre ne peut plus être promu.")
                    return
                end
                ...
                User:notify("error", "membre défini comme" .. rank_name) -- .. " old " .. old_rank_name)
            end
        end
    end
)
    ...
    function(org_id, target_member_id)
        ...
        if User then
            ...
            if Character then
                ...
                if target_member_rank == -1 then
                    User:notify("error", "Le membre est déjà au rang le plus bas.")
                    return
                end
                ...
                User:notify("error", "membre défini comme " .. rank_name)
            end
        end
    end
)
