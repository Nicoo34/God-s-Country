                ...
                if not hasIlegalOrg then
                    ...
                    if Inventory:removeItem(-1, "money", _ILEGAL_ORG_CREATION_PRICE) then
                        ...
                        if org_id then
                            ...
                            User:notify("Une erreur s'est produite lors de la création de votre organisation :(")
                            Inventory:addItem("money", _ILEGAL_ORG_CREATION_PRICE)
                        end
                    else
                        User:notify("Sans argent")
                    end
                else
                    User:notify("Vous êtes déjà dans une organisation")
                end
            end
        end
    end
