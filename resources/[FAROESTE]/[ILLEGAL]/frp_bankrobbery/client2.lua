            ...
            if interiorIndexPlayerIsIn ~= nil then
                ...
                if weaponHash ~= hashUnarmed then
                    if interiorIndexBeingRobbed == nil then
                        if not shootingToStartCooldown then
                            notify("Tirez pour lancer l'assaut.")
                            if IsPedShooting(ped) then
                                initShootingCountdown()
                                notify("trouve un moyen d'ouvrir la salle des coffres !")
                ...
                if interiorIndexPlayerIsIn == interiorIndexBeingRobbed then
                    if secondsUntilRobberyEnds ~= nil then
                        ...
                        drawText(minutes .. " minutes et " .. seconds .. " secondes", true)
                    end
                end
            else
                if secondsUntilAbandonRobbery ~= nil then
                    drawText("~r~rentré à l'intérieur de la banque! " .. math.floor((secondsUntilAbandonRobbery / 10)) .. " seconde restante", true)
                end
            end
        end
    end
    ...
    Citizen.CreateThread(
        function()
            ...
            if doorexplosed == false then
                DrawTxt("Appuyer sur E pour dynamité la porte ", 0.85, 0.95, 0.4, 0.4, true, 255, 255, 255, 255, true, 10000)
...
AddEventHandler(
    ...
    function(index, asParticipant, seconds)
        ...
        TriggerEvent("FRP:TOAST:New", "alert", "le vol se terminera dans " .. seconds .. " secondes")
    end
)
