...
AddEventHandler(
	...
	if User ~= nil then
		if Inventory:getItemAmount('opio') >= quantidade[_source] then
			...
			User:notify("error", "tu dois " .. quantidade[_source].. "d'Opium.")
		end
	end
end)
...
AddEventHandler(
	...
	if User ~= nil then
		...
		--	vRPclient._playSound(player,"CONFIRM_BEEP","HUD_MINI_GAME_SOUNDSET")
			TriggerClientEvent("FRP:TOAST:New", PoliceON[i].getSource(), "alert", "Nous avons reçu un rapport de trafic d'opium, vérifiez ce qui s'est passé.")
