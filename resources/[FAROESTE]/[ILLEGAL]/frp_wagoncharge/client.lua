...
AddEventHandler('FRP:WAGONCHARGE:StartNotify', function(CoordsSpawn, Destiny)
	...
	Wait(3000)  
	TriggerEvent('FRP:NOTIFY:Simple', 'Il y a un wagon de contrebande en cours de transport, il sera intercepté et livré à destination.', 10000)
	...
	--if not (cAPI.hasGroupOrInheritance('trooper') or cAPI.hasGroupOrInheritance('sheriff'))	 then
		...
		Citizen.InvokeNative(0x9CB1A1623062F402, blip, "contrebande d'armes")
	--end
end)
		...
		if not FinishedDelivery then	
			if IsVehicleModel(VehiclePed, GetHashKey("STAGECOACH004X")) then	
				if not MultiRoute then
					...
					TriggerEvent('FRP:NOTIFY:Simple', 'Livrer le chariot à destination.', 10000)
				end
				MultiRoute = true
			else
				MultiRoute = false
