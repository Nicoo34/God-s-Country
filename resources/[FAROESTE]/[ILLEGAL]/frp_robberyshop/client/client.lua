		...
		while true do
			...
			for k, v in pairs(Config.ATMS) do
				...
				if isRobberyActive then
					DrawText("Il reste " .. robberyTime .. " secondes~w~ pour finir le braquage", 0.85, 0.96, 0.4, 0.4, false, 255, 255, 255, 120, 1, 7)
					if robberyTime < 30 then
						DrawText("Appuyer sur " .. Config.robberyCancelKey .. " pour annuler le cambriolage", 0.85, 0.93, 0.4, 0.4, false, 255, 255, 255, 120, 1, 7)
					...
					if distance <= 1.2 then
						DrawText3D(v.x, v.y, v.z + 0.5, "Appuyer sur [ENTER] pour lancer le braquage")
						...
						if IsControlJustPressed(0, 0xC7B5340A) and not IsPedInAnyVehicle(player) then
							if NetworkIsSessionActive() == 1 then
								...
								cAPI.notify("error", "tu es tout seul!")
							end
						end
					end
				end
...
AddEventHandler(
	...
	function(AtmX, AtmY, AtmZ)
		...
		TriggerEvent("FRP:NOTIFY:Simple", "SHERIFF:<br>Vol dans un magasin ! Allez à l'endroit et arrêtez les voleurs dans " .. zone, 10000)
