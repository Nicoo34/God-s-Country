...
AddEventHandler(
	...
	function(atmInfo)
		...
		if #PoliceON < Config.copsRequired then
			TriggerClientEvent("FRP:NOTIFY:Simple", source, "Ce magasin ne peut pas être cambriolé, il n'y a pas de police disponible.", 10000)
		elseif (os.time() - Config.ATMS[atmInfo[1]]["wasRobbed"]) <= Config.robberyInterval then
			...
			TriggerClientEvent("FRP:NOTIFY:Simple", source, "Cette boutique est vide, veuillez patienter " .. timeInterval .. timeUnit, 10000)
		else
...
AddEventHandler(
	...
	function(money)
		...
		TriggerClientEvent("FRP:NOTIFY:Simple", source, "Vous avez volé " .. money .. ".", 10000)
...
AddEventHandler(
	...
	function(money)
		...
		TriggerClientEvent("FRP:ROBREG:cancelfreeze", source)
		TriggerClientEvent("FRP:NOTIFY:Simple", source, "Le braquage a été annulé, tu viens d'avoir $ " .. money .. ".", 10000)
